...
from ..utils.base import humanize_size, run_parallel, random_id, sort_as_sql
...
class BaseDataset:
    ...
    def _read_file(
        ...
        elif format in [".arrow", ".ipc", ".feather"]:
            with filesystem.open_input_file(path) as f:
    ...
    def _write_file(
        ...
        **kwargs,
        ...
        if format == ".parquet":
            pq.write_table(table, path, filesystem=filesystem, schema=schema, **kwargs)
...
class Dataset(BaseDataset):
    ...
    def partition_by(
        ...
        drop:bool=False):
        if as_dict:    
            return dict(partition_by(self.ddb.rel, self._timestamp_column, columns=columns, strftime=strftime, timedelta=timedelta, n_rows=n_rows, as_dict=as_dict, drop=drop))
        
        return list(partition_by(self.ddb.rel, self._timestamp_column, columns=columns, strftime=strftime, timedelta=timedelta, n_rows=n_rows, as_dict=as_dict, drop=drop))
    ...
    def iter_batches(
        ...
        if file_size:
            ...
            yield from partition_by(self.ddb.rel, self._timestamp_column, columns=columns, strftime=strftime, timedelta=timedelta, n_rows=n_rows, as_dict=as_dict, drop=drop)
